...
def process_with_groq_and_qdrant(message):
    try:
        is_job_related = classify_query(message)
        if is_job_related:
            ...
            prompt = f"""Basándote en la siguiente información y la pregunta del usuario, proporciona una respuesta útil y concisa sobre ofertas de trabajo.
...
Respuesta:"""
            ...
            prompt = f"""El usuario ha hecho una pregunta general que no está relacionada con ofertas de trabajo. Por favor, proporciona una respuesta útil y concisa.
...
def classify_query(query):
    response = groq_client.chat.completions.create(
        model="llama3-70b-8192",
        messages=[
            {"role": "system", "content": "Eres un clasificador. Determina si la siguiente consulta está relacionada con búsqueda de trabajo o información laboral. Responde solo con 1 si es si  o 0 si es no."},
